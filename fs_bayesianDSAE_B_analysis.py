...
OUT_DIR = f"outputs_new/bayesianDSAE_B{LB}_{UB}"
B_VALUES = np.arange(LB, UB+INC, INC)
...
result_df = {**{"outer_seed":[], "exe_time":[], "B":[], "b":[], "re_mean":[]}, **{"re_"+feat:[] for feat in feats}, **{"label":[]}} # {**dict1, **dict2,...} is a way to merge multiple dictionaries
    ...
    for B in B_VALUES:
        ...
        for b in range(B):
            ...
            for re_row, label in zip(re_test, y_test):
                result_df["outer_seed"].append(i)
                result_df["exe_time"].append(exe_time)
                result_df["B"].append(B)
                result_df["b"].append(b)
                result_df["re_mean"].append(re_row.mean().item())
        
                for feat, re_feat in zip(feats, re_row):
                    result_df["re_"+feat].append(re_feat.item())
        
                result_df["label"].append(label.item())
    
            _df = pd.DataFrame(result_df)
            grp_mean_df = _df[(_df.outer_seed==i)&(_df.B==B)&(_df.b==b)].groupby(by=["label"]).mean()
    
            if verbose:
                print("B=", B, "b=", b, "normal_mse=",grp_mean_df.loc[0].re_mean, "anomaly_mse=", grp_mean_df.loc[1].re_mean, "anomaly_mse>normal_mse=", grp_mean_df.loc[1].re_mean>grp_mean_df.loc[0].re_mean)
        ...
        rank_df.to_csv(os.path.join(OUT_DIR, f"rank_df{i}_{B}.csv"), index=False)
