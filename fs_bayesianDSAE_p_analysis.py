...
OUT_DIR = f"outputs_new/bayesianDSAE_P{LB}_{UB}"
P_VALUES = np.arange(LB, UB+INC, INC)
...
results_df = {**{"outer_seed":[], "exe_time":[], "P":[], "b":[], "re_mean":[]}, **{"re_"+feat:[] for feat in feats}, **{"label":[]}} # {**dict1, **dict2,...} is a way to merge multiple dictionaries
...
for i in tqdm(range(NUM_REPEATS), position=0):
    ...
    for P in P_VALUES:
        bayesian_dsae = utils.bayesianAutoencoder(input_dim, encoder_layers=encoder_layers, latent_dim=latent_dim, activation_fn = activation_fn, dropout_prob=P)
        ...
        _df = pd.DataFrame(results_df)
        grp_mean_df = _df[(_df.outer_seed==i)&(_df.P==P)].groupby(by=["label"]).mean()
        ...
        rank_df.to_csv(os.path.join(OUT_DIR, f"rank_df{i}_{P}.csv"), index=False)
