...
FS_METHODS = ["random", "filter/mannwhitneyu", "filter/mrmr_classif", "filter/mutual_info_classif", "embedded/LASSO", "filter/singleAE", "filter/bayesianAE"]
...
for fs_method in tqdm(FS_METHODS):
    for i in range(NUM_DATA_PERTURBATIONS):
        for j in range(i+1, NUM_DATA_PERTURBATIONS):
            ...
            map_idx = (i * NUM_DATA_PERTURBATIONS) + j
...
top_k = 5
freq_feats_df = {"fs_method":[], "freq_feats":[]}
...
for fs_method in FS_METHODS:
    ...
    freq_feats_df["fs_method"].append(fs_method)
    freq_feats_df["freq_feats"].append(sorted(freq_df.sort_values(by="frequency", ascending=False).head(top_k).feature.to_list()))
    

freq_feats_df = pd.DataFrame(freq_feats_df)

# display(freq_feats_df)
...
# freq_feats_df = freq_feats_df[~freq_feats_df["fs_method"].isin(["random"])].reset_index(drop=True)
...
# pivot = pivot.reindex([method for method in FS_METHODS if "random" not in method])
