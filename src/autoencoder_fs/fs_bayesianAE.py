...
def bayesian_dsae_fs(df, mc_samples):
    ...
    del bayesian_dsae, model, X_train, y_train, X_test, y_test, train_ds, val_ds, dls #during the next call of this function, the decorator will be called, which will flush the cache
...
def main():
    ...
    rank_dict, elapsed_time, peak_mem, cpu_mem, gpu_mem = bayesian_dsae_fs(radiomics_df, mc_samples=B)
    rank_df = pd.DataFrame(rank_dict)
    rank_df.sort_values("rank", inplace=True)
    rank_df.reset_index(drop=True, inplace=True)
