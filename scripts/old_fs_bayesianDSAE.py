...
results_df = {**{"outer_seed":[], "exe_time":[], "cpu_memory":[], "gpu_memory":[], "b":[], "re_mean":[]}, **{"re_"+feat:[] for feat in feats}, **{"label":[]}} # {**dict1, **dict2,...} is a way to merge multiple dictionaries
...
for i in tqdm(range(NUM_REPEATS), position=0, desc="running bayesianDSAE"):
    ...
    #the train v/s test pids changes with each repeat
    ...
    for b in range(B):
        ...
        for re_row, label in zip(re_test, y_test):
            ...
            results_df["cpu_memory"].append(cpu_mem)
            results_df["gpu_memory"].append(gpu_mem)
...
#     _results_df[_results_df.outer_seed==i].to_csv(os.path.join(OUT_DIR, f"results_df{i}.csv"))
